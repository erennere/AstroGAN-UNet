#!/bin/bash
#SBATCH --job-name=prepare-plots-scenarios
#SBATCH --output=../logs/prepare_plots_scenarios_%j.out
#SBATCH --error=../errs/prepare_plots_scenarios_%j.err
...
init_aun_paths
setup_aun_logging "prepare_plots_scenarios"
activate_aun_env
enter_aun_src
...
run_one() {
    ...
    echo "[prepare_plots] idx=${EVAL_INDEX} cw=${EVAL_CONCURRENT_WORKERS} model=${model} attn=${attn} scaling=${scaling} loss=${loss} model_alias=${MODEL_ALIAS_SELECTOR}"
