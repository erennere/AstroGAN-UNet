#!/bin/bash
#SBATCH --job-name=prepare-images-scenarios
#SBATCH --output=../logs/prepare_images_scenarios_%j.out
#SBATCH --error=../errs/prepare_images_scenarios_%j.err
...
init_aun_paths
setup_aun_logging "prepare_images_scenarios"
activate_aun_env
enter_aun_src
...
run_one() {
    ...
    echo "[prepare_images] idx=${EVAL_INDEX} cw=${EVAL_CONCURRENT_WORKERS} model=${model} attn=${attn} scaling=${scaling} loss=${loss} model_alias=${MODEL_ALIAS_SELECTOR}"
