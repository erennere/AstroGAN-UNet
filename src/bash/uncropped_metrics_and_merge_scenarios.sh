#!/bin/bash
#SBATCH --job-name=uncropped-scenarios
#SBATCH --output=../logs/uncropped_scenarios_%j.out
#SBATCH --error=../errs/uncropped_scenarios_%j.err
...
init_aun_paths
setup_aun_logging "uncropped_scenarios"
activate_aun_env
enter_aun_src
...
run_one() {
    ...
    echo "[uncropped] idx=${EVAL_INDEX} cw=${EVAL_CONCURRENT_WORKERS} model=${model} attn=${attn} scaling=${scaling} loss=${loss} model_alias=${MODEL_ALIAS_SELECTOR}"
