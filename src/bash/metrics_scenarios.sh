#!/bin/bash
#SBATCH --job-name=metrics-scenarios
#SBATCH --output=../logs/metrics_scenarios_%j.out
#SBATCH --error=../errs/metrics_scenarios_%j.err
...
init_aun_paths
setup_aun_logging "metrics_scenarios"
activate_aun_env
enter_aun_src
...
# Runtime selectors consumed by parse_runtime_selector_cli_args.
...
run_one() {
    ...
    echo "[metrics] idx=${EVAL_INDEX} cw=${EVAL_CONCURRENT_WORKERS} model=${model} attn=${attn} scaling=${scaling} loss=${loss} model_alias=${MODEL_ALIAS_SELECTOR}"
